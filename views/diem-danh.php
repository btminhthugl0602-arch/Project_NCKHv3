...
<head>
    ...
    <style>
        body { font-family: system-ui, -apple-system, sans-serif; background: #f8f5ff; min-height: 100vh; }
        .dd-card {
            background: #fff;
        ...
        .dd-meta-label { color: #94a3b8; min-width: 100px; font-size: .78rem; }
        .dd-meta-value { color: #1e1b4b; font-weight: 600; flex: 1; }
        .dd-btn {
        ...
        .dd-btn-primary:hover { opacity: .9; }
        ...
        #dd-gps-info { display: none; }
        #dd-status-msg { margin-top: .75rem; font-size: .82rem; text-align: center; }
    </style>
</head>
...
<div class="dd-card">
    <div class="dd-header">
        ...
        <p style="opacity:.85;font-size:.85rem;margin:0"><?= $lichTrinh ? htmlspecialchars($lichTrinh['tenHoatDong']) : '' ?></p>
    </div>
    <div class="dd-body">
        <div class="dd-success-icon">
            <span class="material-symbols-outlined" style="color:#fff;font-size:40px">check_circle</span>
        </div>
        <h3 style="text-align:center;font-size:1.1rem;font-weight:700;color:#1e1b4b;margin-bottom:.25rem">Đã điểm danh!</h3>
...
<div class="dd-card">
    ...
    <div class="dd-body">
        ...
        <div id="dd-gps-info" style="background:#f0fdf4;border-radius:.75rem;padding:.75rem 1rem;margin-bottom:1rem;font-size:.8rem;color:#166534;">
...
const DDPage = (() => {
    ...
    function startDiemDanh() {
        ...
        btn.disabled = true;
        btn.innerHTML = `<span class="material-symbols-outlined" style="animation:spin 1s linear infinite;display:inline-block">refresh</span> Đang xử lý...`;
        ...
        if (navigator.geolocation) {
            ...
            navigator.geolocation.getCurrentPosition(
                pos => {
                    ...
                    document.getElementById('dd-gps-text').textContent = `${_lat.toFixed(5)}, ${_lng.toFixed(5)}`;
                    submit('GPS');
                },
                () => submit('THU_CONG'),
    ...
    async function submit(phuongThuc) {
        try {
            ...
            if (_lat !== null) { payload.vi_tri_lat = _lat; payload.vi_tri_lng = _lng; }
            ...
            if (json.status === 'success') {
                ...
                if (btn2) {
                    btn2.disabled = false;
                    btn2.innerHTML = '<span class="material-symbols-outlined">how_to_reg</span> Xác nhận điểm danh';
                }
