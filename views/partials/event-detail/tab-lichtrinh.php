...
<style>
/* ── Layout ── */
.lt-header {display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;}
.lt-timeline {display:flex;flex-direction:column;gap:.75rem;}
...
.lt-item:active{cursor:grabbing;}
.lt-item.dragging{opacity:.5;border-color:#9213ec;}
.lt-item.drag-over{border-color:#9213ec;box-shadow:0 0 0 3px rgba(146,19,236,.15);}

.lt-item-header {
    display:flex;align-items:center;gap:.75rem;
    padding:.875rem 1.25rem;
}
.lt-drag-handle {color:#d1c4e9;cursor:grab;flex-shrink:0;}
.lt-time-badge {
    font-size:.72rem;font-weight:600;color:#7c3aed;
    background:#f3e8ff;padding:.2rem .6rem;border-radius:99px;
...
.lt-type-HOAT_DONG{background:#e0f2fe;color:#0369a1;}
.lt-type-DIEM_DANH{background:#fce7f3;color:#be185d;}
.lt-type-NGHI     {background:#f0fdf4;color:#15803d;}
.lt-type-KHAC     {background:#f8fafc;color:#64748b;}

.lt-item-name  {font-weight:600;color:#1e1b4b;flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.lt-item-place {font-size:.78rem;color:#94a3b8;margin-left:auto;white-space:nowrap;}
.lt-item-actions{display:flex;gap:.35rem;flex-shrink:0;}
.lt-btn-icon {
    background:none;border:none;cursor:pointer;padding:.35rem;
    border-radius:.5rem;color:#94a3b8;transition:background .15s, color .15s;
    display:flex;align-items:center;font-size:18px;
}
.lt-btn-icon:hover{background:#f3f4f6;color:#7c3aed;}
.lt-btn-icon.danger:hover{background:#fef2f2;color:#dc2626;}
...
.lt-expand-panel.open{display:block;}

.lt-phien-empty {
    text-align:center;padding:1.5rem;
    background:#fdf8ff;border-radius:.75rem;
    border:1.5px dashed #ddd6fe;
}
.lt-phien-card {
    border-radius:.75rem;padding:1rem;
    background:#fff;border:1.5px solid #e8e0f7;
}
...
.phien-CHUAN_BI {background:#fef9c3;color:#92400e;}
.phien-DANG_MO  {background:#dcfce7;color:#166534;}
.phien-DA_DONG  {background:#f1f5f9;color:#64748b;}

/* ── Attendance table ── */
.lt-att-table {width:100%;border-collapse:collapse;font-size:.82rem;margin-top:.75rem;}
.lt-att-table th{background:#f8f5ff;color:#7c3aed;font-weight:600;padding:.5rem .75rem;text-align:left;border-bottom:1px solid #ede9fe;}
.lt-att-table td{padding:.5rem .75rem;border-bottom:1px solid #f8f5ff;color:#374151;}
.lt-att-table tr:last-child td{border-bottom:none;}

/* ── Buttons ── */
.lt-btn {
    display:inline-flex;align-items:center;gap:.35rem;
    padding:.5rem 1rem;border-radius:.625rem;border:none;cursor:pointer;
...
.lt-btn-primary {
    ...
    color:#fff;box-shadow:0 3px 12px rgba(146,19,236,.3);
}
.lt-btn-primary:hover{opacity:.9;}
.lt-btn-success {background:#22c55e;color:#fff;}
.lt-btn-success:hover{opacity:.9;}
.lt-btn-warning {background:#f59e0b;color:#fff;}
.lt-btn-warning:hover{opacity:.9;}
.lt-btn-danger  {background:#ef4444;color:#fff;}
.lt-btn-danger:hover{opacity:.9;}
.lt-btn-outline {
    background:transparent;border:1.5px solid #ddd6fe;color:#7c3aed;
}
.lt-btn-outline:hover{background:#f3e8ff;}
...
.lt-modal-backdrop.open{opacity:1;pointer-events:all;}
.lt-modal {
    background:#fff;border-radius:1.25rem;width:min(720px,95vw);
    max-height:90vh;overflow-y:auto;
    box-shadow:0 20px 60px rgba(146,19,236,.18);
    transform:translateY(16px);transition:transform .2s;
}
.lt-modal-backdrop.open .lt-modal{transform:translateY(0);}
.lt-modal-header {
    display:flex;align-items:center;justify-content:space-between;
    padding:1.25rem 1.5rem;border-bottom:1px solid #f0eaf8;
    position:sticky;top:0;background:#fff;z-index:1;
}
.lt-modal-title{font-weight:700;color:#1e1b4b;font-size:1rem;}
.lt-modal-body {padding:1.5rem;}
.lt-modal-footer{
    padding:1rem 1.5rem;border-top:1px solid #f0eaf8;
    display:flex;justify-content:flex-end;gap:.75rem;
    position:sticky;bottom:0;background:#fff;
}

/* ── Form ── */
.lt-form-row{display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1rem;}
.lt-form-row.full{grid-template-columns:1fr;}
.lt-form-group label{display:block;font-size:.8rem;font-weight:600;color:#4b4580;margin-bottom:.35rem;}
.lt-form-group input,
.lt-form-group select,
.lt-form-group textarea {
    width:100%;padding:.6rem .875rem;border:1.5px solid #e5dff5;
    border-radius:.625rem;font-size:.85rem;color:#1e1b4b;
    background:#fdfcff;transition:border-color .15s;
    box-sizing:border-box;
}
.lt-form-group input:focus,
.lt-form-group select:focus,
.lt-form-group textarea:focus {
    outline:none;border-color:#9213ec;
    box-shadow:0 0 0 3px rgba(146,19,236,.1);
}
.lt-form-group textarea{resize:vertical;min-height:70px;}
.lt-form-required{color:#dc2626;margin-left:.2rem;}

/* ── Leaflet map ── */
#lt-map-picker{height:250px;border-radius:.75rem;overflow:hidden;border:1.5px solid #e5dff5;margin-top:.5rem;}
.lt-map-hint{font-size:.75rem;color:#94a3b8;margin-top:.3rem;}

/* ── Search ── */
.lt-search-box{position:relative;}
.lt-search-box input{padding-left:2.25rem;}
.lt-search-icon{position:absolute;left:.7rem;top:50%;transform:translateY(-50%);color:#9ca3af;font-size:18px;}
.lt-search-results {
    position:absolute;top:100%;left:0;right:0;
    background:#fff;border:1.5px solid #e5dff5;border-radius:.75rem;
    box-shadow:0 8px 24px rgba(0,0,0,.1);z-index:100;max-height:200px;overflow-y:auto;
    display:none;
}
.lt-search-results.open{display:block;}
.lt-search-item {
    padding:.6rem 1rem;cursor:pointer;font-size:.83rem;color:#374151;
    display:flex;align-items:center;gap:.5rem;
    transition:background .1s;
}
.lt-search-item:hover{background:#fdf8ff;}

/* ── QR Display ── */
#lt-qr-container{text-align:center;padding:1rem;}
#lt-qr-container canvas{border-radius:.75rem;box-shadow:0 4px 20px rgba(146,19,236,.1);}

/* ── Skeleton ── */
.lt-skeleton{
    background:linear-gradient(90deg,#f0eef5 25%,#e8e4f0 50%,#f0eef5 75%);
    background-size:200% 100%;animation:lt-shimmer 1.4s infinite;border-radius:.5rem;
}
...
.lt-toast-msg{
    padding:.75rem 1.25rem;border-radius:.75rem;font-size:.83rem;font-weight:600;
    box-shadow:0 4px 20px rgba(0,0,0,.12);animation:lt-slide-in .25s ease;
    display:flex;align-items:center;gap:.5rem;max-width:340px;
}
@keyframes lt-slide-in{from{transform:translateX(120%)}to{transform:translateX(0)}}
.lt-toast-success{background:#22c55e;color:#fff;}
.lt-toast-error  {background:#ef4444;color:#fff;}
.lt-toast-info   {background:#7c3aed;color:#fff;}
</style>
...
const LT = (() => {
    ...
    function buildItem(item) {
        ...
        const loaiMap = { HOAT_DONG: 'Hoạt động', DIEM_DANH: 'Điểm danh', NGHI: 'Nghỉ', KHAC: 'Khác' };
    ...
    async function submitForm() {
        ...
        if (!ten || !batDau) { toast('Vui lòng điền tên và thời gian bắt đầu', 'error'); return; }
        if (loai === 'DIEM_DANH' && !ketThuc) { toast('Hoạt động điểm danh cần có thời gian kết thúc', 'error'); return; }
    ...
    async function deleteItem(id) {
        ...
        try {
            const res = await api('/api/su_kien/xoa_lich_trinh.php', 'POST', { id_sk: ID_SK, id_lich_trinh: id });
            if (res.status === 'success') {
    ...
    async function moPhien(idPhienDD, idSk) {
        try {
            const res = await api('/api/su_kien/mo_phien_diemdanh.php', 'POST', { id_sk: idSk, id_phien_dd: idPhienDD });
            if (res.status === 'success') {
    ...
    async function dongPhien(idPhienDD, idSk) {
        ...
        try {
            const res = await api('/api/su_kien/dong_phien_diemdanh.php', 'POST', { id_sk: idSk, id_phien_dd: idPhienDD });
            if (res.status === 'success') {
    ...
    function moLaiPhien(idPhienDD, idSk) { moPhien(idPhienDD, idSk); }

    // ── QR Code ──────────────────────────────────────────────
    function showQR(token, idPhienDD) {
        if (!token) { toast('Không có token QR', 'error'); return; }
    ...
    function searchUser(input, idPhienDD) {
        ...
        _searchTimer = setTimeout(async () => {
            try {
                const res = await fetch(`${BASE}/api/nhom/tim_kiem_user.php?q=${encodeURIComponent(q)}&id_sk=${ID_SK}`);
    ...
    async function saveOrder() {
        ...
        const payload = {
            id_sk: ID_SK,
            items: items.map((el, i) => ({ id_lich_trinh: parseInt(el.dataset.id), thu_tu: i + 1 }))
        };
        try {
    ...
    function initMap() {
        ...
        const defaultLat = 10.7769, defaultLng = 106.7009; // HCMC
        ...
        ['lt-f-lat','lt-f-lng'].forEach(id => {
            document.getElementById(id).addEventListener('change', () => {
                ...
                if (la && ln) { setMarker(la, ln); _map.setView([la, ln], 17); }
            });
        });
    }
    ...
    function setMarker(lat, lng) {
        ...
        _marker = L.marker([lat, lng], { draggable: true })
            .addTo(_map)
    ...
    function getMyLocation() {
        if (!navigator.geolocation) { toast('Trình duyệt không hỗ trợ geolocation', 'error'); return; }
        navigator.geolocation.getCurrentPosition(pos => {
            const lat = pos.coords.latitude, lng = pos.coords.longitude;
            document.getElementById('lt-f-lat').value = lat.toFixed(7);
            document.getElementById('lt-f-lng').value = lng.toFixed(7);
            if (!_map) initMap();
            else { setMarker(lat, lng); _map.setView([lat, lng], 17); }
    ...
    return {
        load, openModal, closeModal, submitForm, deleteItem,
        toggleExpand, createPhien, moPhien, dongPhien, moLaiPhien,
        showQR, viewAttendance, closeAttModal,
        searchUser, diemDanhThuCong,
        getMyLocation,
        _toast: toast,
    };
})();
