...
<style>
/* ── Timeline SV ── */
.sv-timeline { display: flex; flex-direction: column; gap: .75rem; }
...
.sv-type-HOAT_DONG { background: #e0f2fe; color: #0369a1; }
.sv-type-DIEM_DANH { background: #fce7f3; color: #be185d; }
.sv-type-NGHI      { background: #f0fdf4; color: #15803d; }
.sv-type-KHAC      { background: #f8fafc; color: #64748b; }

.sv-item-name  { font-weight: 600; color: #1e1b4b; flex: 1; min-width: 120px; }
.sv-item-place { font-size: .78rem; color: #94a3b8; white-space: nowrap; }
.sv-item-right { display: flex; align-items: center; gap: .75rem; margin-left: auto; flex-wrap: wrap; }
...
.sv-phien-not-open {
    font-size: .78rem; color: #f59e0b; font-weight: 600;
    display: flex; align-items: center; gap: .35rem;
}
.sv-already-checked {
    font-size: .8rem; color: #16a34a; font-weight: 600;
    display: flex; align-items: center; gap: .35rem;
    background: #f0fdf4; padding: .35rem .875rem;
...
.sv-btn-dd {
    ...
    padding: .5rem 1.1rem; border-radius: .625rem; border: none;
    cursor: pointer; font-size: .82rem; font-weight: 700;
    background: linear-gradient(135deg, #be185d, #9213ec);
    color: #fff; box-shadow: 0 3px 12px rgba(146,19,236,.28);
    transition: opacity .15s, transform .1s;
}
.sv-btn-dd:hover  { opacity: .9; }
.sv-btn-dd:active { transform: scale(.97); }
.sv-btn-dd:disabled { opacity: .5; cursor: not-allowed; }
...
.sv-btn-login:hover { background: #f3e8ff; }
...
.sv-gps-modal {
    ...
    width: min(400px, 94vw); padding: 2rem;
    text-align: center;
...
.sv-toast-msg {
    padding: .7rem 1.2rem; border-radius: .75rem; font-size: .83rem; font-weight: 600;
    box-shadow: 0 4px 20px rgba(0,0,0,.12); animation: sv-slide-in .25s ease;
    display: flex; align-items: center; gap: .5rem; max-width: 320px;
}
@keyframes sv-slide-in { from{transform:translateX(120%)} to{transform:translateX(0)} }
.sv-toast-success { background: #22c55e; color: #fff; }
.sv-toast-error   { background: #ef4444; color: #fff; }
.sv-toast-info    { background: #7c3aed; color: #fff; }
</style>
...
const LTSV = (() => {
            ...
            navigator.geolocation.getCurrentPosition(
                ...
                _err => {
                    ...
                    if (confirm('Không lấy được vị trí GPS.\nBạn có muốn điểm danh không cần xác minh vị trí không?')) {
                        ...
                        if (btn) { btn.disabled = false; btn.innerHTML = '<span class="material-symbols-outlined" style="font-size:18px">how_to_reg</span> Điểm danh ngay'; }
                    }
                },
    ...
    async function submitDiemDanh(idPhienDD, phuongThuc, lat, lng, btn) {
        try {
            ...
            if (lat !== null) { payload.vi_tri_lat = lat; payload.vi_tri_lng = lng; }
